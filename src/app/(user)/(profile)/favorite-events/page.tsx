...
const FavoriteEvents: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        await Promise.all([fetchUserProfile(), fetchfavoriteEvents(), fetchNotifications()])
      } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 md:ml-64">
        <Navbar sidebarOpen={sidebarOpen} toggleSidebar={toggleSidebar} pageTitle="Favorite Events" />

        <div className="pt-16 p-6">
          <div className="mb-8 bg-white rounded-lg p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-3xl font-bold text-gray-800 mb-2">Favorite Events</h2>
                <p className="text-gray-600">
                  {favoriteEvents.length > 0
                    ? `Showing ${indexOfFirstEvent + 1}-${Math.min(indexOfLastEvent, favoriteEvents.length)} of ${favoriteEvents.length} favorite events`
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      <div className="flex items-center text-gray-600">
                        ...
                        <span className="text-sm ml-2">{event.teamLeaderNumber}</span>
                      </div>
                    </div>

                    <div className="flex justify-between items-center pt-4 border-t">
                      {event.isblocked || event.isperformerblockedevents ? (
                        <span className="text-red-600 text-sm font-semibold">Cannot book</span>
                      ) : (
                        <button

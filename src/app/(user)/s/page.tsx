  ...
  const handleSendMessage = async () => {
    if (newMessage.trim() && selectedChatRoom) {
      try {
        ...
        const isSocketConnected = socketRef.current?.connected || false;

        console.log('fay',isSocketConnected)
        console.log('Socket connected:', isSocketConnected); // Check if this is being correctly logged
  
        await axiosInstance.post(
          `/handleSendMessage/${userProfile.id}/${selectedChatRoom.otherId}`,
          { message: newMessage, isSocketConnected } // Send the socket status
        );

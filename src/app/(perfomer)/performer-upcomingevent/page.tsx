...
const EventCard: React.FC<{ event: PerformerUpcomingEvent; onCancel: (eventId: string, eventDate: string) => void; cancelConfirmation: { [key: string]: boolean }; setCancelConfirmation: React.Dispatch<React.SetStateAction<{ [key: string]: boolean }>> }> = ({
  event,
  onCancel,
  cancelConfirmation,
...
const CancelEventButton: React.FC<{
  ...
  setCancelConfirmation: React.Dispatch<React.SetStateAction<{ [key: string]: boolean }>>;
  ...
  return (
    <button
      onClick={() => setCancelConfirmation(prev => ({...prev, [event._id]: true}))}
  ...
  const paginationEvent = async (page: number) => {
    try {
      ...
      if (response.data.events) {
        const formattedEvents = response.data.events.map((event: PerformerUpcomingEvent) => ({
          ...event,
      ...
      {events.length === 0 ? (
        ...
        <>
          ...
          <div className="flex justify-center items-center space-x-2 mt-6">
            ...
            <button
              onClick={() => paginationEvent(Math.min(totalPages, currentPage + 1))}

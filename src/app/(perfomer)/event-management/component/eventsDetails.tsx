...
const EventManagementDashboard: React.FC = () => {
  ...
  const handleEditEvent = (eventId: string) => {
    router.push(`/event-management/eventupdate/${eventId}`);
  };

  const handleDeleteEvent = async (eventId: string) => {
    if (window.confirm('Are you sure you want to delete this event?')) {
      try {
        const response=await axiosInstance.post(`/deleteEvent/${eventId}`);
        if(response.status==200){
          const updatedEvents = events.filter(event => event._id !== eventId);
          setEvents(updatedEvents); // Make sure setEvents is exposed in your store
        }
            ...
            {filteredEvents.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredEvents.map(event => (
                  <EventCard
                    key={event._id || `${event.title}-${event.teamLeader}`}
                    event={event}

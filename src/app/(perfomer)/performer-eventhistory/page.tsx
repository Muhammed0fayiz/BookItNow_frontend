...
// Custom error type
interface TimeParsingError extends Error {
  message: string;
}
...
const formatTime = (timeString: string): string => {
  try {
    ...
    console.log('Time parsing error:', error as TimeParsingError);
    return timeString;
  }
};
...
const formatDate = (dateString: string): string => {
  try {
    ...
    console.log('Date parsing error:', error as TimeParsingError);
...
const UpcomingEvents: React.FC = () => {
    const { performerEvents } = useEventHistory();
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 6;
  
    const totalPages = Math.ceil(performerEvents.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentEvents = performerEvents.slice(startIndex, endIndex);
  ...
  const PerformerEventHistory: React.FC = () => {
    const router = useRouter();
    const {fetchAllEvents } = useEventHistory();
  ...
  export default PerformerEventHistory;

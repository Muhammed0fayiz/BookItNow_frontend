...
const CheckoutForm: React.FC<CheckoutFormProps> = ({ onPaymentSuccess, onPaymentError, amount, onBack }) => {
  const stripe = useStripe();
  const elements = useElements();
  const [processing, setProcessing] = useState<boolean>(false);
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4">
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={onBack}
          className="flex items-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 font-medium text-gray-700"
        >
        ...
        <button
          type="submit"
          disabled={!stripe || processing}
          className={`flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold 
            ${processing ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90 transition-all duration-300 transform hover:scale-105'}`}
        >
          {processing ? (
            <div className="flex items-center justify-center">
...
const EventPayment: React.FC<EventPaymentProps> = ({ amount, onSuccess, onError, show, onBack }) => {
  const [clientSecret, setClientSecret] = useState<string | null>(null);
